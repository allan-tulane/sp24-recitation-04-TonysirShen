...
def word_count_reduce(group):
  result = reduce(plus, 0, group[1])
  return (group[0], result)


"""
    Params:
      group...a list of the form (token, list_of_ones), indicating the number of times each word appears.
    Returns:
...
def sentiment_map(doc,
  ...
  for word in doc.split():
    if word in pos_terms:
      lst.append(('positive', 1))
    elif word in neg_terms:
      lst.append(('negative', 1))
  return lst


## parallelize function, just fir fun
def sentiment_map_parallel(
    doc,
    pos_terms=set(['good', 'great', 'awesome', 'sockdolager']),
  ...
  def evaluate_word(word):
    ...
    return None
    ### map sequence
  result = [evaluate_word(w) for w in words]
...
test_string = "apple on tree apple on ground"
print(test_string.split())
print(word_count_map(test_string))
print(word_count_reduce(['i', [1, 1, 1]]))
print(sentiment_map_parallel('it was a terrible waste of time'))
